...
def start_ec_daemon(docker_ip: str, docker_services: pytest_docker.plugin.Services) -> None:
    ...
    shutil.copyfile("oregano/tests/regtest/configs/electron-cash-config", _datadir + "/regtest/config")
...
def docker_compose_file(pytestconfig) -> str:
    ...
    return os.path.join(str(pytestconfig.rootdir), "oregano/tests/regtest/docker-compose.yml")
